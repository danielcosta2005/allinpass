alter table "public"."notification_jobs" drop constraint "notification_jobs_project_id_fkey";

alter table "public"."notifications" drop constraint "notifications_project_id_fkey";

alter table "public"."notification_jobs"
  add constraint "notification_jobs_project_id_fkey"
...
alter table "public"."notification_jobs"
  validate constraint "notification_jobs_project_id_fkey";
    ...
    SELECT
      ...
      (
        ...
        WHERE wl.project_id = p_project_id
          AND wl.created_at >= p_start_date
          AND wl.created_at <= p_end_date
      ) AS wallet_linked
    FROM public.events
...
CREATE OR REPLACE FUNCTION public.fn_link_member_by_email(
  p_email text,
  p_project uuid,
...
BEGIN
  ...
  WHERE project_id = p_project
    AND google_sub = p_google_sub;

  IF v_id IS NULL THEN
    INSERT INTO public.customers(project_id, google_sub, email, name)
    VALUES (p_project, p_google_sub, p_email, p_name)
...
CREATE OR REPLACE FUNCTION public.get_wallet_link_and_customer_points(
  p_project_id uuid,
  p_google_sub text
)
 RETURNS TABLE(
...
BEGIN
  ...
  FROM public.customers c
  LEFT JOIN public.wallet_links wl
    ON wl.customer_id = c.id
   AND wl.project_id = p_project_id
  WHERE c.project_id = p_project_id
    AND c.google_sub = p_google_sub
  LIMIT 1;
  ...
  IF v_ls_id IS NULL THEN
    ...
    IF v_first_visit IS NULL THEN
      ...
      RETURNING id, points, cycle_start, cycle_end
      INTO v_ls_id, points, cycle_start, cycle_end;
    ELSE
      SELECT count(*)
        INTO v_visits_count
    ...
    IF cycle_end IS NOT NULL AND v_now::date >= cycle_end THEN
      ...
      IF v_first_visit IS NULL THEN
        ...
        SET points = 0,
            ...
            cycle_end = (CURRENT_DATE + 30),
            updated_at = now()
        WHERE id = v_ls_id
        RETURNING points, cycle_start, cycle_end
        INTO points, cycle_start, cycle_end;
      ELSE
        SELECT count(*)
          INTO v_visits_count
        FROM public.visits
        ...
        UPDATE public.loyalty_states
        SET points = points,
            cycle_start = v_first_visit::date,
            cycle_end = (v_first_visit::date + 30),
            updated_at = now()
        WHERE id = v_ls_id
        RETURNING points, cycle_start, cycle_end
        INTO points, cycle_start, cycle_end;
    ...
    ELSE
      ...
      IF v_visits_count >= 10 THEN
        ...
        VALUES (
          ...
          'reward_unlocked',
          NULL,
          jsonb_build_object('reason','auto_redeem','visits', v_visits_count),
          now()
        );
      ELSE
...
AS $function$
BEGIN
  INSERT INTO public.wallet_configs_history (
    wallet_config_id,
    project_id,
    pass_template,
    reason
...
CREATE OR REPLACE FUNCTION public.update_wallet_link_google_object_id(
  p_wallet_link_id uuid,
  p_google_object_id text
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'wallet_link_id', p_wallet_link_id,
    'google_object_id', p_google_object_id
  );
