...
describe("billing trial expiration enforcement", () => {
  test("keeps the trial expiration scheduler and exposes an expired-trial lock state in /org", () => {
    ...
    expect(trialExpiredStateSource).toContain("Acesso operacional pausado");
    expect(trialExpiredStateSource).toContain("Fale com o suporte");
    expect(trialExpiredStateSource).not.toContain("Escolher plano");
    expect(trialExpiredStateSource).not.toContain("plano pago");
    expect(trialExpiredStateSource).not.toContain("onOpenPlanChange");
  });
